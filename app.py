...
class HandDetectionApp:
    ...
    def open_chat_window(self):
        ...
        self.chat_canvas = tk.Canvas(chat_window, width=self.vid.get(cv2.CAP_PROP_FRAME_WIDTH),
                                     height=self.vid.get(cv2.CAP_PROP_FRAME_HEIGHT))
        self.chat_canvas.grid(row=0, column=1, rowspan=2, padx=10, pady=10)

        self.btn_send_signal = tk.Button(chat_window, text="Enviar Sinal de Libras", command=self.detect_libras)
        self.btn_send_signal.grid(row=2, column=1, pady=10)
    ...
    def check_libras_in_chat(self, frame):
        ...
        if response.status_code == 200:
            ...
            for item in predictions:
                if item['tagName'] == 'Oi' and item['probability'] > max_probability:
                    max_probability = item['probability']
                    max_tag = item['tagName']

            if max_probability >= 0.1:
                self.update_chat_log(f"Sinal detectado: {max_tag}")
                self.generate_response("Oi, tudo bem? Como você está?")
            else:
                self.update_chat_log("Não entendi, faça novamente")

    def update_chat_log(self, message):
        """
        Atualiza o log de chat na interface gráfica.
    ...
    def generate_response(self, user_input):
        """
        Gera uma resposta para exibir no log de chat.

        Args:
            user_input (str): Entrada do usuário para gerar a resposta.
        """
        # Aqui você pode modificar para gerar respostas baseadas em regras ou lógica programada
        self.update_chat_log(f"IA: {user_input}")

    def send_message(self, event):
        """
        Envia a mensagem digitada pelo usuário para o log de chat.
        ...
        self.update_chat_log(f"Você: {message}")
        self.generate_response(message)

    def on_closing(self):
        """
        Finaliza a aplicação ao fechar a janela principal.
        """
        if self.vid.isOpened():
            self.vid.release()
        if hasattr(self, 'chat_vid') and self.chat_vid.isOpened():
